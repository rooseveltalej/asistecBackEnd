...
def get_user_events_route(user_id: str, db: Session = Depends(get_db)):
...
def update_event_route(event_id: str, event: schemas.EventCreate, db: Session = Depends(get_db)):
